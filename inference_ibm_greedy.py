...
class Decoder(ABC):
    def decode(self, model, input_ids, *args, **kwargs):
        raise NotImplemented
    
    def generate(self, inputs, model_id, client, params):
        outputs = []
        for idx, response in enumerate(client.text.generation.create(
            model_id=model_id, inputs=inputs, parameters=params
            )):
...
def get_generations_bam_greedy(
    model_name: str,
    prompts: list[str],
    max_new_tokens: int,
    batch_size: int,
...
def main():
    if args.prompts_dir != None:
        for filename in glob.glob(f"{args.prompts_dir}/*.csv"):
            if not ('question_lang:en' in filename or 'question_lang_en' in filename):
                print(f"Skipping {filename}, non-English question_lang")
                continue
            ...
            if (
                ...
                prompts_df[["id", f"predictions_{args.model_name}"]].to_pickle(
                    os.path.join(args.output_dir, filename.split("/")[-1]).replace('.csv','.pkl')
                )
    return
